...
export default function FeedPage() {
  ...
  const loadPosts = async () => {
    ...
    if (followsError) {
      setMsg(`Follows load error: ${followsError.message}`);
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <button onClick={() => setFeedMode("for_you")} disabled={feedMode === "for_you"}>
          For You
        </button>
        <button onClick={() => setFeedMode("following")} disabled={feedMode === "following"}>
          Following
        </button>
      </div>
            ...
            <div key={p.id} style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12 }}>
              ...
              >
                <div style={{ fontSize: 12, color: "#666" }}>
                  {a.username ? (
                    <Link href={`/u/${encodeURIComponent(a.username)}`}>{a.label}</Link>
                  ) : (
                    a.label
                  )}{" "}
              ...
              <div style={{ display: "grid", gap: 8, marginBottom: 10 }}>
                {p.comments.map((c) => {
                  ...
                  const ca = formatAuthor(c.profile, c.user_id);

                  return (
                    <div key={c.id} style={{ border: "1px solid #333", borderRadius: 6, padding: 8 }}>
                      <div style={{ fontSize: 12, color: "#888", marginBottom: 4 }}>
                        {ca.username ? (
                          <Link href={`/u/${encodeURIComponent(ca.username)}`}>{ca.label}</Link>
                        ) : (
                          ca.label
                        )}{" "}
