...
function trim(text: string, max = 1800) {
...
export async function GET(req: NextRequest) {
  ...
  if (plainRes.status !== 200 || briefRes.status !== 200) {
    return NextResponse.json(
      { ok: false, error: "report_bundle_imessage_unavailable", statuses: { plain: plainRes.status, brief: briefRes.status } },
      { status: 500 }
    );
  }
