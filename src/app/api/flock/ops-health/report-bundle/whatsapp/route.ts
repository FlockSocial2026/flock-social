...
export async function GET(req: NextRequest) {
  ...
  if (bundleRes.status !== 200 || briefRes.status !== 200) {
    ...
    if (summaryRes.status === 200) {
      ...
      briefRes = { status: 200, json: { brief: summaryLine } };
    }
  }

  if (bundleRes.status !== 200 || briefRes.status !== 200) {
    return NextResponse.json(
      { ok: false, error: "report_bundle_whatsapp_unavailable", statuses: { bundle: bundleRes.status, brief: briefRes.status } },
      { status: 500 }
    );
  }

  const bundle = bundleRes.json;
  const exec = String(bundle?.bundle?.executive?.reportText || "");
  const hourly = String(bundle?.bundle?.hourly?.report || "");
  const overnight = String(bundle?.bundle?.overnight?.report || "");
