...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1062 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/whatsapp`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1062-report-bundle-whatsapp-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasText: typeof json?.text === "string" && json.text.includes("Flock Ops Report Bundle"),
      hasBold: typeof json?.text === "string" && json.text.includes("*Summary:*"),
      hasCharCount: typeof json?.charCount === "number" && json.charCount > 20,
    };

    if (res.status !== 200 || !summary.hasText || !summary.hasBold || !summary.hasCharCount) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1062 report bundle whatsapp smoke failed assertions");
    }
