...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1057 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/discord`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1057-report-bundle-discord-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasContent: typeof json?.content === "string" && json.content.includes("Ops"),
      hasEmbeds: Array.isArray(json?.embeds) && json.embeds.length > 0,
      hasTitle: Array.isArray(json?.embeds) && String(json.embeds[0]?.title || "").includes("Flock Ops Report Bundle"),
    };

    if (res.status !== 200 || !summary.hasContent || !summary.hasEmbeds || !summary.hasTitle) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1057 report bundle discord smoke failed assertions");
    }
