...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1064 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/signal`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1064-report-bundle-signal-smoke" },
    });
    ...
    if (res.status !== 200 || !summary.hasText || !summary.hasBrief || !summary.hasCharCount) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1064 report bundle signal smoke failed assertions");
    }
