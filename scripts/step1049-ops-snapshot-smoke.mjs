...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1049 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/snapshot`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1049-ops-snapshot-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasCompactText: typeof json?.compactText === "string" && json.compactText.includes("Ops"),
      hasSnapshot: typeof json?.snapshot?.critical === "number" && typeof json?.snapshot?.warning === "number",
    };

    if (res.status !== 200 || !summary.hasCompactText || !summary.hasSnapshot) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1049 snapshot smoke failed assertions");
    }
