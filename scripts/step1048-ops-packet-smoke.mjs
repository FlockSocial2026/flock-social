...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1048 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/packet`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1048-ops-packet-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasVersion: typeof json?.packetVersion === "string",
      hasSummary: Boolean(json?.packet?.summary?.ok),
      hasHandoff: Boolean(json?.packet?.handoff?.ok),
      hasMarkdown: Boolean(json?.packet?.markdown?.ok),
    };

    if (res.status !== 200 || !summary.hasVersion || !summary.hasSummary || !summary.hasHandoff || !summary.hasMarkdown) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1048 packet smoke failed assertions");
    }
