...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1037 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const { data: event, error: eventErr } = await admin
      .from("church_events")
      .insert({ church_id: church.id, author_user_id: churchAdmin.id, title: `Step1037 Event ${runId}`, starts_at: startsAt })
      .select("id,title")
      .single();
    if (eventErr || !event) throw new Error(`event insert failed: ${eventErr?.message || "unknown"}`);
    createdEventIds.push(event.id);

    const { error: snapErr } = await admin.from("flock_event_rsvp_snapshots").insert({
      church_id: church.id,
      event_id: event.id,
      going: 2,
      maybe: 1,
      not_going: 0,
      total: 3,
      maybe_to_going_pct: 67,
    });
    if (snapErr) throw new Error(`snapshot insert failed: ${snapErr.message}`);

    const { error: dispatchErr } = await admin.from("flock_dispatch_logs").insert({
      church_id: church.id,
      actor_user_id: churchAdmin.id,
      event_id: event.id,
      event_title: event.title,
      audience: "all",
      cadence: "T-24h",
    });
    if (dispatchErr) throw new Error(`dispatch insert failed: ${dispatchErr.message}`);

    const res = await fetch(`${baseUrl}/api/flock/ops-health`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1037-ops-health-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      snapshotHealthy: Boolean(json?.snapshot?.healthy),
      snapshotAgeMin: json?.snapshot?.ageMin ?? null,
      dispatch24Total: json?.dispatchLast24h?.total ?? 0,
      upcoming24h: json?.upcomingEvents?.next24h ?? 0,
    };

    if (res.status !== 200 || summary.dispatch24Total < 1 || summary.upcoming24h < 1 || summary.snapshotAgeMin === null) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1037 ops-health smoke failed assertions");
    }
