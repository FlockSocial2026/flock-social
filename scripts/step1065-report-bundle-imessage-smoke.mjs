...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1065 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/imessage`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1065-report-bundle-imessage-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasBubble: typeof json?.bubble === "string" && json.bubble.includes("Flock Ops Report Bundle"),
      hasBrief: typeof json?.brief === "string" && json.brief.includes("Ops"),
      hasCharCount: typeof json?.charCount === "number" && json.charCount > 10,
    };

    if (res.status !== 200 || !summary.hasBubble || !summary.hasBrief || !summary.hasCharCount) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1065 report bundle imessage smoke failed assertions");
    }
