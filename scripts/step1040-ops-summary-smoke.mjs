...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1040 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/summary`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1040-ops-summary-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      healthyFlag: Boolean(json?.status?.healthy !== undefined),
      hasHealthPayload: Boolean(json?.health?.ok),
      hasGuidancePayload: Boolean(json?.guidance?.ok),
      hasAlertsPayload: Boolean(json?.alerts?.ok),
    };

    if (res.status !== 200 || !summary.healthyFlag || !summary.hasHealthPayload || !summary.hasGuidancePayload || !summary.hasAlertsPayload) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1040 ops summary smoke failed assertions");
    }
