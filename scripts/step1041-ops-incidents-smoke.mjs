...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1041 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const { data: event, error: eventErr } = await admin
      .from("church_events")
      .insert({ church_id: church.id, author_user_id: churchAdmin.id, title: `Step1041 Event ${runId}`, starts_at: startsAt })
      .select("id")
      .single();
    if (eventErr || !event) throw new Error(`event insert failed: ${eventErr?.message || "unknown"}`);
    createdEventIds.push(event.id);

    const res = await fetch(`${baseUrl}/api/flock/ops-health/incidents`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1041-ops-incidents-smoke" },
    });
    ...
    const summary = {
      ...
      hasSeverity: incidents.every((i) => i.severity === "critical" || i.severity === "warning"),
    };

    if (res.status !== 200 || summary.openCount !== summary.incidentCount || !summary.hasSeverity) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1041 ops incidents smoke failed assertions");
    }
