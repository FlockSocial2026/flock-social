...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1051 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/hourly-report`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1051-hourly-report-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasReport: typeof json?.report === "string" && json.report.includes("Hourly Incremental Progress Report"),
      hasCadence: typeof json?.cadence?.["T-24h"] === "number",
      hasActions: Array.isArray(json?.topActions),
    };

    if (res.status !== 200 || !summary.hasReport || !summary.hasCadence || !summary.hasActions) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1051 hourly report smoke failed assertions");
    }
