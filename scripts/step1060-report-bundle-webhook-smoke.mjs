...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1060 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/webhook`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1060-report-bundle-webhook-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasHeaders: typeof json?.headers?.["x-flock-event-signature-sha256"] === "string",
      hasPayloadType: String(json?.payload?.type || "") === "flock.ops.report_bundle.v1",
      hasBrief: typeof json?.payload?.brief === "string",
    };

    if (res.status !== 200 || !summary.hasHeaders || !summary.hasPayloadType || !summary.hasBrief) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1060 report bundle webhook smoke failed assertions");
    }
