...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1058 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/telegram`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1058-report-bundle-telegram-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasParseMode: json?.parseMode === "MarkdownV2",
      hasText: typeof json?.text === "string" && json.text.includes("Flock Ops Report Bundle"),
      hasSummary: typeof json?.text === "string" && json.text.includes("Summary"),
    };

    if (res.status !== 200 || !summary.hasParseMode || !summary.hasText || !summary.hasSummary) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1058 report bundle telegram smoke failed assertions");
    }
