...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1067 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/catalog`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1067-report-bundle-catalog-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasItems: Array.isArray(json?.items) && json.items.length >= 10,
      hasHealthyCount: typeof json?.healthy === "number",
      hasTotal: typeof json?.total === "number" && json.total >= 10,
    };

    if (res.status !== 200 || !summary.hasItems || !summary.hasHealthyCount || !summary.hasTotal) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1067 report bundle catalog smoke failed assertions");
    }
