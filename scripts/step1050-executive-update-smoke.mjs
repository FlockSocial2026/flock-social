...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1050 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/executive-update`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1050-executive-update-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasHeadline: typeof json?.headline === "string",
      hasReportText: typeof json?.reportText === "string" && json.reportText.includes("Status:"),
      hasTopActions: Array.isArray(json?.topActions),
    };

    if (res.status !== 200 || !summary.hasHeadline || !summary.hasReportText || !summary.hasTopActions) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1050 executive update smoke failed assertions");
    }
