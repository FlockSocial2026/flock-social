...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1052 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/overnight-report`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1052-overnight-report-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasReport: typeof json?.report === "string" && json.report.includes("Overnight Full Report"),
      hasPacketVersion: typeof json?.packetVersion === "string",
      hasTopActions: Array.isArray(json?.topActions),
    };

    if (res.status !== 200 || !summary.hasReport || !summary.hasPacketVersion || !summary.hasTopActions) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1052 overnight report smoke failed assertions");
    }
