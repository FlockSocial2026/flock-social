...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1066 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/json`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1066-report-bundle-json-smoke" },
    });
    ...
    const summary = {
      ...
      hasBrief: typeof json?.brief === "string",
      hasBundle: typeof json?.bundle === "object" && json?.bundle !== null,
    };

    if (res.status !== 200 || !summary.hasSummaryLine || !summary.hasBrief || !summary.hasBundle) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1066 report bundle json smoke failed assertions");
    }
