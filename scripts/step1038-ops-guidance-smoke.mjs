...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1038 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/explain`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1038-ops-guidance-smoke" },
    });

    const json = await res.json();
    const actions = json?.guidance?.recommendedActions;
    const hasCadenceGuidance = Boolean(json?.guidance?.dispatchCoverage?.["T-24h"]);
    ...
    if (res.status !== 200 || summary.actionsCount < 3 || !hasCadenceGuidance) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1038 ops guidance smoke failed assertions");
    }
