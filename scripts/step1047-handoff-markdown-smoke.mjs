...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1047 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/handoff/markdown`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1047-handoff-markdown-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasMarkdown: typeof json?.markdown === "string" && json.markdown.includes("# Flock Ops Handoff"),
      hasHeadlineLine: typeof json?.markdown === "string" && json.markdown.includes("## Headline"),
    };

    if (res.status !== 200 || !summary.hasMarkdown || !summary.hasHeadlineLine) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1047 handoff markdown smoke failed assertions");
    }
