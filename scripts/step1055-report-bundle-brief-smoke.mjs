...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1055 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/brief`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1055-report-bundle-brief-smoke" },
    });
    ...
    const summary = {
      ...
      hasLen: typeof json?.maxLen === "number",
      under280: typeof json?.maxLen === "number" ? json.maxLen <= 280 : false,
    };

    if (res.status !== 200 || !summary.hasBrief || !summary.hasLen || !summary.under280) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1055 report bundle brief smoke failed assertions");
    }
