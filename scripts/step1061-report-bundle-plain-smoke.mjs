...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1061 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/plain`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1061-report-bundle-plain-smoke" },
    });
    ...
    const summary = {
      ...
      hasLines: Array.isArray(json?.lines) && json.lines.length >= 5,
      hasCharCount: typeof json?.charCount === "number" && json.charCount > 20,
    };

    if (res.status !== 200 || !summary.hasText || !summary.hasLines || !summary.hasCharCount) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1061 report bundle plain smoke failed assertions");
    }
