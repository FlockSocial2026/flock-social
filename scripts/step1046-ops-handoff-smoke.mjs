...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1046 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/handoff`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1046-ops-handoff-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasHandoffText: typeof json?.handoffText === "string" && json.handoffText.includes("Headline:"),
      hasBriefPayload: Boolean(json?.brief?.ok),
      hasRunbookPayload: Boolean(json?.runbook?.ok),
    };

    if (res.status !== 200 || !summary.hasHandoffText || !summary.hasBriefPayload || !summary.hasRunbookPayload) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1046 handoff smoke failed assertions");
    }
