...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1043 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/escalations`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1043-ops-escalation-smoke" },
    });
    const json = await res.json();

    const protocol = Array.isArray(json?.protocol) ? json.protocol : [];
    const summary = {
      runId,
      status: res.status,
      hasLevel: ["none", "watch", "escalate"].includes(json?.level),
      protocolCount: protocol.length,
      hasCounts: typeof json?.counts?.critical === "number" && typeof json?.counts?.warning === "number",
    };

    if (res.status !== 200 || !summary.hasLevel || summary.protocolCount === 0 || !summary.hasCounts) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1043 escalation smoke failed assertions");
    }
