...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1063 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/sms`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1063-report-bundle-sms-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasSms: typeof json?.sms === "string" && json.sms.includes("Ops"),
      hasCharCount: typeof json?.charCount === "number" && json.charCount > 10,
      hasSegments: typeof json?.segmentsEstimate === "number" && json.segmentsEstimate >= 1,
    };

    if (res.status !== 200 || !summary.hasSms || !summary.hasCharCount || !summary.hasSegments) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1063 report bundle sms smoke failed assertions");
    }
