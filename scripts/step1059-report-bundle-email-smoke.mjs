...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1059 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/email`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1059-report-bundle-email-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasSubject: typeof json?.subject === "string" && json.subject.includes("Flock Ops"),
      hasTextBody: typeof json?.textBody === "string" && json.textBody.includes("Flock Ops Report Bundle"),
      hasHtml: typeof json?.htmlBody === "string" && json.htmlBody.includes("<h1>Flock Ops Report Bundle</h1>"),
    };

    if (res.status !== 200 || !summary.hasSubject || !summary.hasTextBody || !summary.hasHtml) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1059 report bundle email smoke failed assertions");
    }
