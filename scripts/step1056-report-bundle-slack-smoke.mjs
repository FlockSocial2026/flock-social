...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1056 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/slack`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1056-report-bundle-slack-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasText: typeof json?.text === "string" && json.text.includes("Ops"),
      hasBlocks: Array.isArray(json?.blocks) && json.blocks.length >= 3,
      hasHeader: Array.isArray(json?.blocks) && json.blocks.some((b) => b?.type === "header"),
    };

    if (res.status !== 200 || !summary.hasText || !summary.hasBlocks || !summary.hasHeader) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1056 report bundle slack smoke failed assertions");
    }
