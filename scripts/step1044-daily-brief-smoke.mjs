...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1044 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/daily-brief`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1044-daily-brief-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasHeadline: typeof json?.headline === "string" && json.headline.length > 0,
      hasMetrics: typeof json?.metrics?.openIncidents === "number",
      topActionsCount: Array.isArray(json?.topActions) ? json.topActions.length : 0,
    };

    if (res.status !== 200 || !summary.hasHeadline || !summary.hasMetrics) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1044 daily brief smoke failed assertions");
    }
