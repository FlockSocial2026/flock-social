...
(async () => {
  try {
    ...
    const { data: church, error: churchErr } = await admin
      .from("churches")
      .insert({ name: `Step1054 Church ${runId}`, slug: `${runId}-church` })
      .select("id")
      .single();
    ...
    const res = await fetch(`${baseUrl}/api/flock/ops-health/report-bundle/markdown`, {
      headers: { authorization: `Bearer ${token}`, "user-agent": "step1054-report-bundle-markdown-smoke" },
    });
    ...
    const summary = {
      runId,
      status: res.status,
      hasMarkdown: typeof json?.markdown === "string" && json.markdown.includes("# Flock Ops Report Bundle"),
      includesExecutive: typeof json?.markdown === "string" && json.markdown.includes("## Executive Update"),
      includesOvernight: typeof json?.markdown === "string" && json.markdown.includes("## Overnight Report"),
    };

    if (res.status !== 200 || !summary.hasMarkdown || !summary.includesExecutive || !summary.includesOvernight) {
      console.log(JSON.stringify({ summary, json }, null, 2));
      throw new Error("Step1054 report bundle markdown smoke failed assertions");
    }
